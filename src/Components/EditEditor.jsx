...
const EditEditor = ({onUpdate, comment, commentId, handleEditEditor, replyingTo}) => {
  ...
  return (
    <div className="flex flex-col min-w-[100%] rounded gap-3 sm:gap-4">
      <Editor onEdit={handleContent} status={commentUpdated} >{replyingTo? "@"+replyingTo+comment: comment}</Editor>
      <button aria-label="Click to Update Comment" onClick={handleUpdate} className="bg-moderate-blue hover:bg-light-grayish-blue text-white rounded p-1 w-20 h-fit sm:h-7 md:h-10 sm:order-2 font-[500] self-end">UPDATE</button>
    </div>
  )
}
